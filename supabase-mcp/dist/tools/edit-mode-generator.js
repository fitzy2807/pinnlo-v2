...
function mapBlueprintType(blueprintType) {
    const mappings = {
...
async function performGeneration(args) {
    const { cardId, blueprintType, cardTitle, strategyId, userId, existingFields = {} } = args;
    try {
        ...
        if (detectedStrategyId && contextConfig && contextConfig.length > 0) {
            const context = await getCachedContext(`${userId}-${detectedStrategyId}-${blueprintType}`, () => gatherContext(detectedStrategyId, userId, contextConfig));
        ...
        const userPrompt = buildGenerationPrompt(blueprintType, cardTitle, existingFields, contextSummary);
...
Ensure all required fields are populated and follow the exact field names and types specified above.`;
        ...
        await trackGeneration(userId, cardId, blueprintType, contextCards, promptConfig.system_prompt, mergedFields, completion.usage?.total_tokens || 0, Date.now() - startTime, promptConfig.model_preference || 'gpt-4o-mini');
...
async function gatherContext(strategyId, userId, contextConfig) {
    const contextCards = [];
...
async function summarizeCards(cards, summarizationPrompt) {
    const cardsContent = cards.map(card => `Card: ${card.title}\nDescription: ${card.description || 'None'}\nData: ${JSON.stringify(card.card_data || {})}`).join('\n\n---\n\n');
    try {
        const completion = await getOpenAIClient().chat.completions.create({
...
function buildGenerationPrompt(blueprintType, cardTitle, existingFields, contextSummary) {
    const hasExistingContent = Object.keys(existingFields).some(key => existingFields[key] && String(existingFields[key]).trim().length > 0);
...
function mergeFields(existing, generated) {
...
// Track generation in history
async function trackGeneration(userId, cardId, blueprintType, contextCards, promptUsed, fieldsGenerated, tokensUsed, generationTimeMs, modelUsed) {
    try {
...
async function getCachedContext(cacheKey, fetchFn) {
...
async function processVoiceContext(transcript, blueprintType, existingFields, strategyId) {
    try {
...
async function analyzeVoiceThemes(transcript, blueprintType) {
    const themes = [];
...
async function mapVoiceToFields(transcript, blueprintType, existingFields) {
    const mappings = {};
    const transcriptLower = transcript.toLowerCase();
...
async function generateVoiceContextSummary(transcript, themes, strategyId) {
...
Generate the SUBSTANTIALLY IMPROVED field content as a JSON object using voice context as primary strategic input.`;
        ...
        await trackVoiceGeneration(userId, cardId, blueprintType, transcript, voiceEditSystemPrompt, updatedFields, completion.usage?.total_tokens || 0, Date.now() - startTime, promptConfig.model_preference || 'gpt-4o-mini', voiceContext);
...
async function trackVoiceGeneration(userId, cardId, blueprintType, transcript, promptUsed, fieldsGenerated, tokensUsed, generationTimeMs, modelUsed, voiceContext) {
    try {
        await getSupabaseClient().from('ai_generation_history').insert({
            user_id: userId,
            card_id: cardId,
