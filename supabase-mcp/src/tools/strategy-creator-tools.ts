...
function generateUserPrompt(contextSummary: string, targetBlueprint: string, count: number, existingCards: any[], config?: any) {
...
export async function generateContextPreview(contextSummary: string, targetBlueprint: string, existingCards: any[]) {
  try {
    // Try to get system prompt from database  
    const dbPrompt = await getSystemPrompt('blueprint', targetBlueprint);
    if (dbPrompt) {
