...
// Handler functions
export async function handleGenerateTechStackComponent(args: any) {
  ...
  const stackContext = existing_stack.length > 0 
    ? `Current tech stack includes: ${existing_stack.map((t: any) => `${t.technology_name} (${t.category})`).join(', ')}.`
...
export async function handleAnalyzeTechStack(args: any) {
  const { components, project_type = "Web Application" } = args;
  
  // Analyze current stack
  const categories = [...new Set(components.map((c: any) => c.category))];
...
Current Stack:
${components.map((c: any) => `- ${c.technology_name} (${c.category}) - ${c.implementation_status}`).join('\n')}
