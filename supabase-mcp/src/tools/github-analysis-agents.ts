...
export class RepositoryExplorer {
  private github: { headers: GitHubHeaders };
   ...
   */
  async explore(): Promise<ExplorerResult> {
   ...
   */
  private async getRepositoryInfo(): Promise<RepositoryData> {
   ...
   */
  private async getAllFiles(): Promise<Array<{path: string, type: string, size: number}>> {
   ...
   */
  private prioritizeFiles(allFiles: Array<{path: string, type: string, size: number}>): string[] {
    ...
    allFiles.forEach(file => {
      if (prioritizedFiles.size >= 50) return; // Limit to avoid GitHub API rate limits
   ...
   */
  private async fetchFileContents(filePaths: string[]): Promise<FileContent[]> {
   ...
   */
  private buildDirectoryStructure(allFiles: Array<{path: string, type: string, size: number}>): string[] {
...
export const repositoryExplorerTool = {
  ...
  handler: async ({ repository_url, github_token, user_id }: any) => {
    try {
...
export class TechnologyAnalyzer {
  private explorerData: ExplorerResult;
   ...
   */
  async analyze(): Promise<StructuredTechStack> {
    ...
    try {
      ...
      const result: StructuredTechStack = {
   ...
   */
  private analyzeConfigurations(): Record<string, any> {
   ...
   */
  private detectFrameworks(dependencies: Record<string, string>, configurations: Record<string, any>): string[] {
    ...
    if (dependencies['bootstrap']) frameworks.push('Bootstrap');
    if (dependencies['@mui/material']) frameworks.push('Material-UI');
    ...
    if (dependencies['rollup']) frameworks.push('Rollup');

    // Database/ORM
    if (dependencies['prisma']) frameworks.push('Prisma');
   ...
   */
  private categorizeTechnologies(dependencies: Record<string, string>, configurations: Record<string, any>, frameworks: string[]): TechnologiesByCategory {
    ...
    if (dependencies['next']) technologies.frontend.push(`Next.js-${dependencies['next'].replace(/[\^\~]/, '')}`);
    if (dependencies['react']) technologies.frontend.push(`React-${dependencies['react'].replace(/[\^\~]/, '')}`);
    if (dependencies['vue']) technologies.frontend.push(`Vue.js-${dependencies['vue'].replace(/[\^\~]/, '')}`);
    if (dependencies['tailwindcss']) technologies.frontend.push(`Tailwind-CSS-${dependencies['tailwindcss'].replace(/[\^\~]/, '')}`);
    if (dependencies['@headlessui/react']) technologies.frontend.push(`Headless-UI-${dependencies['@headlessui/react'].replace(/[\^\~]/, '')}`);
    if (dependencies['framer-motion']) technologies.frontend.push(`Framer-Motion-${dependencies['framer-motion'].replace(/[\^\~]/, '')}`);
    if (dependencies['lucide-react']) technologies.frontend.push(`Lucide-React-${dependencies['lucide-react'].replace(/[\^\~]/, '')}`);

    // Backend technologies
    if (dependencies['@supabase/supabase-js']) technologies.backend.push(`Supabase-${dependencies['@supabase/supabase-js'].replace(/[\^\~]/, '')}`);
    if (dependencies['express']) technologies.backend.push(`Express-${dependencies['express'].replace(/[\^\~]/, '')}`);
    if (dependencies['fastify']) technologies.backend.push(`Fastify-${dependencies['fastify'].replace(/[\^\~]/, '')}`);
    if (dependencies['next'] && !dependencies['express']) technologies.backend.push('Next.js-API-Routes');
    ...
    if (dependencies['prisma']) technologies.database.push(`Prisma-${dependencies['prisma'].replace(/[\^\~]/, '')}`);
    if (dependencies['pg']) technologies.database.push(`PostgreSQL-${dependencies['pg'].replace(/[\^\~]/, '')}`);
    ...
    // AI technologies
    if (dependencies['openai']) technologies.ai.push(`OpenAI-${dependencies['openai'].replace(/[\^\~]/, '')}`);
    if (dependencies['@anthropic-ai/sdk']) technologies.ai.push(`Anthropic-${dependencies['@anthropic-ai/sdk'].replace(/[\^\~]/, '')}`);
    if (dependencies['langchain']) technologies.ai.push(`LangChain-${dependencies['langchain'].replace(/[\^\~]/, '')}`);

    // Development tools
    if (dependencies['typescript']) technologies.development.push(`TypeScript-${dependencies['typescript'].replace(/[\^\~]/, '')}`);
    if (dependencies['eslint']) technologies.development.push(`ESLint-${dependencies['eslint'].replace(/[\^\~]/, '')}`);
    if (dependencies['prettier']) technologies.development.push(`Prettier-${dependencies['prettier'].replace(/[\^\~]/, '')}`);
    if (dependencies['jest']) technologies.development.push(`Jest-${dependencies['jest'].replace(/[\^\~]/, '')}`);

    // Platforms
    if (dependencies['@supabase/supabase-js']) technologies.platforms.push('Supabase-Platform');
    if (dependencies['stripe']) technologies.platforms.push('Stripe-Platform');

    // Integrations
    if (dependencies['@supabase/auth-helpers-nextjs']) technologies.integrations.push('Supabase-Auth');
    if (dependencies['@vercel/analytics']) technologies.integrations.push('Vercel-Analytics');
   ...
   */
  private analyzeInfrastructure(): InfrastructureInfo {
   ...
   * Analyze database setup
   */
  private analyzeDatabase(dependencies: Record<string, string>): DatabaseInfo {
    ...
    if (dependencies['pg']) database.primary.push('PostgreSQL');
    if (dependencies['mysql2']) database.primary.push('MySQL');
    if (dependencies['mongodb']) database.primary.push('MongoDB');

    // ORMs
    if (dependencies['prisma']) database.orm.push('Prisma');
    if (dependencies['sequelize']) database.orm.push('Sequelize');
    if (dependencies['typeorm']) database.orm.push('TypeORM');
   ...
   */
  private detectAIIntegrations(dependencies: Record<string, string>): AIIntegrations {
    console.log('🤖 Agent 2: Detecting AI integrations...');
    
    const ai: AIIntegrations = {
      apis: [],
      frameworks: [],
      models: [],
      integrations: []
    ...
    // API integrations
    if (dependencies['openai']) ai.apis.push('OpenAI');
    if (dependencies['@anthropic-ai/sdk']) ai.apis.push('Anthropic');
    if (dependencies['@google/generative-ai']) ai.apis.push('Google AI');

    // ML frameworks
    if (dependencies['langchain']) ai.frameworks.push('LangChain');
    if (dependencies['@tensorflow/tfjs']) ai.frameworks.push('TensorFlow.js');

    // Check for AI usage in code files
    const codeFiles = this.explorerData.files.filter(f => 
  ...
  private parseComposerJson(content: string): Record<string, string> {
    try {
   ...
   */
  private detectDevelopmentTools(dependencies: Record<string, string>): string[] {
    const tools: string[] = [];
...
export const technologyAnalyzerTool = {
  ...
  handler: async ({ exploration_data, user_id }: any) => {
    try {
...
export class GapAnalysisEngine {
  private techStack: StructuredTechStack;
   ...
   */
  async analyze(): Promise<EnhancedTechStack> {
   ...
   */
  private analyzeFrontendGaps(): GapAnalysis {
   ...
   */
  private analyzeBackendGaps(): GapAnalysis {
   ...
   */
  private analyzeDatabaseGaps(): GapAnalysis {
   ...
   */
  private analyzeInfrastructureGaps(): GapAnalysis {
   ...
   */
  private analyzePlatformGaps(): GapAnalysis {
   ...
   */
  private analyzeAIGaps(): GapAnalysis {
   ...
   */
  private analyzeDevelopmentGaps(): GapAnalysis {
   ...
   */
  private analyzeIntegrationGaps(): GapAnalysis {
   ...
   * Generate migration recommendations
   */
  private generateMigrationNotes(): string[] {
   ...
   */
  private generateStrategicRecommendations(gapAnalysis: Record<string, GapAnalysis>): StrategicRecommendation[] {
   ...
   */
  private enhanceTechnologies(gapAnalysis: Record<string, GapAnalysis>): EnhancedTechnologies {
    const enhanced: EnhancedTechnologies = {
   ...
   */
  private enhanceCategoryTechnologies(technologies: string[], gapAnalysis: GapAnalysis): string[] {
  ...
  private generateFrontendRecommendations(gaps: string[]): string[] {
  ...
  private generateBackendRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generateDatabaseRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generateInfrastructureRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generatePlatformRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generateAIRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generateDevelopmentRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
  ...
  private generateIntegrationRecommendations(gaps: string[]): string[] {
    const recommendations: string[] = [];
...
export const gapAnalysisEngineTool = {
  ...
  handler: async ({ structured_tech_stack, user_id }: any) => {
    try {
...
export class GitHubAnalysisOrchestrator {
   ...
   */
  private extractGapRecommendations(recommendations: string[]): string[] {
