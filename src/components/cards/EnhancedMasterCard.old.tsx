...
function EnhancedMasterCardInternal({
  ...
  // Analytics tracking - moved to top
  ...
  // Initialize auto-save with all features (moved to top to fix hook order)
  const {
    data: localData,
    updateField,
    updateFields,
  ...
  const validationRules = useMemo(() => {
    ...
    // Add validation for required fields only if blueprint exists
    if (blueprint?.fields) {
  ...
  const fieldsBySection = useMemo(() => 
    blueprint?.fields ? groupFieldsBySection(blueprint.fields) : {},
    [blueprint]
  )
  ...
  // Check if blueprint exists after all hooks are declared
  if (!blueprint) {
    return (
      <CardContainer>
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        {!isCollapsed && (
          <>
            {/* Enhanced Preview Mode for Intelligence Cards */}
            {localData.cardType?.includes('intelligence') && !isEditMode && (
              <div className="enhanced-preview-mode transition-all duration-300">
                {/* Intelligence Metrics Header */}
                <div className="px-4 py-3 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-100 shadow-sm">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                    <div className="flex items-center gap-4">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white shadow-sm border border-blue-200 text-blue-800">
                        <div className="w-2 h-2 bg-blue-500 rounded-full mr-2"></div>
                        {localData.cardType?.replace('-intelligence', '').charAt(0).toUpperCase() + localData.cardType?.replace('-intelligence', '').slice(1)} Intelligence
                      </span>
                      <span className="text-xs text-gray-600 font-medium">
                        {new Date(localData.created_at || localData.createdDate).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric'
                        })}
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-600 font-medium">Credibility:</span>
                        <div className="flex items-center gap-2">
                          <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden shadow-inner">
                            <div 
                              ...
                              style={{ width: `${(localData.credibility_score || 5) * 10}%` }}
                            />
                          </div>
                          <span className="text-xs font-bold text-gray-800 min-w-[28px]">{localData.credibility_score || 5}/10</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden shadow-inner">
                            <div 
                              className="h-2 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full transition-all duration-500 ease-out"
                              style={{ width: `${(localData.relevance_score || 5) * 10}%` }}
                            />
                          </div>
                          <span className="text-xs font-bold text-gray-800 min-w-[28px]">{localData.relevance_score || 5}/10</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Executive Summary */}
                <div className="p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <p className="text-sm text-gray-800 leading-relaxed font-medium">
                    {localData.description || localData.summary || 'No summary available'}
                  </p>
                </div>
                  ...
                  <div className="p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full font-medium">
                        {localData.key_findings.length} insight{localData.key_findings.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                    <div className="grid gap-3">
                      {localData.key_findings.map((finding, index) => (
                        <div 
                          key={index}
                          className="group bg-white border border-gray-200 hover:border-blue-300 px-4 py-3 rounded-lg text-sm font-medium shadow-sm hover:shadow-md transition-all duration-200 cursor-default"
                        >
                          <div className="flex items-start gap-3">
                            <div className="flex-shrink-0 w-5 h-5 bg-gradient-to-br from-blue-500 to-indigo-600 rounded text-white text-xs font-bold flex items-center justify-center mt-0.5">
                              {index + 1}
                            </div>
                ...
                {/* Strategic Context - Two Column Layout */}
                <div className="p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200">
                  <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:border-purple-300 hover:shadow-md transition-all duration-200">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                        <h4 className="text-sm font-bold text-gray-800">Strategic Alignment</h4>
                      </div>
                      <p className="text-sm text-gray-700 leading-relaxed">
                        {localData.strategicAlignment || localData.strategic_implications || 'No strategic alignment defined'}
                      </p>
                    </div>
                    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:border-purple-300 hover:shadow-md transition-all duration-200">
                      <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed">
                        {localData.recommended_actions || 'No recommended actions defined'}
                      </p>
                    </div>
                  </div>
                </div>

                {/* Intelligence Content - Collapsible */}
                {localData.intelligence_content && (
                    ...
                    <CollapsibleSection
                      ...
                      preview={
                        <div className="text-xs text-gray-600 mt-1">
                          {localData.intelligence_content.length > 100 
                            ? `${localData.intelligence_content.substring(0, 100)}...` 
                            : localData.intelligence_content
                          }
                        </div>
                      }
                ...
                {/* Source Reference and Additional Metadata */}
                {(localData.source_reference || localData.tags?.length > 0) && (
                  <div className="p-6 bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
                    ...
                    <div className="space-y-3">
                      {localData.source_reference && (
                        <div>
                          <h4 className="text-xs font-medium text-gray-600 mb-1">Source Reference</h4>
                          <p className="text-sm text-gray-800 font-medium">{localData.source_reference}</p>
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {localData.tags.map((tag, index) => (
                              <span 
                                key={index}
                                className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-white border border-gray-300 text-gray-700 hover:border-gray-400 transition-colors"
                              >
                                #{tag}
                              </span>
                            ))}
            ...
            {/* Priority and Confidence Badges - Hide for intelligence cards */}
            {!localData.cardType?.includes('intelligence') && (
            ...
            {/* Description field - always visible for non-intelligence cards */}
            {!localData.cardType?.includes('intelligence') && (
            ...
            {/* Description field for intelligence cards in edit mode */}
            {localData.cardType?.includes('intelligence') && isEditMode && (
              <div className="p-3 border-b border-gray-100">
                <AIEnhancedField
                  label="Executive Summary"
                  value={localData.description || localData.summary || ''}
                  onChange={(value) => handleFieldChange('description', value)}
            ...
            {/* Assessment Section (Priority & Confidence with Rationale) - Hide for intelligence cards */}
            {!localData.cardType?.includes('intelligence') && (
              <CollapsibleSection
                title="Assessment"
                colorScheme="yellow"
