...
export default function TemplateBank({ onClose }: TemplateBankProps) {
  ...
  const handleCreateCard = async () => {
    const newCard = await createCard({
      ...
      card_type: 'template',
      priority: 'medium',
      card_data: { section: selectedSection }
    })
  ...
  const handleQuickAddSubmit = async () => {
    ...
    try {
      await createCard({
        ...
        card_type: 'template',
        priority: 'medium',
        card_data: { source: 'quick_add', section: selectedSection }
      })
  ...
  const filteredCards = cards.filter(card => {
    ...
    if (viewType === 'section') {
      const cardSection = card.card_data?.section || 'section1'
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {viewType === 'section' 
                          ? 'Create a new strategic context card'
