...
export interface OrganisationGroupCard {
  id: string
  group_id: string
  card_id: string
  created_at: string
...
export interface GroupWithCards extends OrganisationGroup {
  cards: any[]
}
...
export function useOrganisationGroups() {
  ...
  const fetchGroups = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('organisation_groups_with_counts')
        .select('*')
        .eq('user_id', user.id)
  ...
  const updateGroup = async (id: string, updates: Partial<OrganisationGroup>) => {
    try {
  ...
  const getGroupCards = async (groupId: string) => {
    try {
      const { data, error } = await supabase
        .from('organisation_group_cards')
        .select(`
          id,
          organisation_cards!inner (
            id,
            title,
            description,
            card_type,
